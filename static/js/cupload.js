	...
	Cupload.prototype = {
		...
		upLoadFileAjax(file) {
			return new Promise((resolve, reject) => {
				...
				if (file_typename !== '.jpg' && file_typename !== '.gif' && file_typename !== '.png' && file_typename !== '.svg') {
						reject('请选择图片文件')
						return
				}
				fetch('/api/upload',{
		...
		createUploadInput: function() {
			...
			this.uploadInput.onchange = function() {
				_this.upLoadFileAjax().then(function(respone) {
					_this.removeUploadBox()
					_this.uploadImage(respone.fileName)
				}).catch(() => {
					alert('文件上传失败')
				})
			}
		...
		uploadImage: function(reslutName) {
			var file = this.uploadInput.files[0]
			this.createUploadBox()
			...
			reader.onload = function(e) {
				_this.limitedWidthAndHeight(e.target.result, file.name, reslutName)
			}
		...
		limitedWidthAndHeight: function(src, name, reslutName) {
			...
			tempImage.onload = function() {
				...
				_this.foreachNum(src, name, this.width, this.height, reslutName)
			}
		},

		foreachNum: function(src, name, width, height, reslutName) {
			this.createImageBox(src, name, width, height, reslutName)
			if (this.imageList.children.length >= this.opt.num) {
				this.removeUploadBox()
			}
		},

		createImageBox: function(src, name, width, height, reslutName, state = true) {
			...
			this.createImagePreview(src, width, height, reslutName)
		...
		createImagePreview: function(src, width, height, reslutName) {
			...
			this.imagePreview[this.i].setAttribute('file-name', reslutName)
